...
class BaselineModel(nn.Module):
    ...
    def __init__(self, in_channels, channels, latent_dim, num_layers, num_transformer_layers, kernel_size, recon_type="mse", content_cosine=False, time_resolution=256, losses=['recon']):
        ...
        self.encoder_bottleneck = TransformerBottleneck(channels, num_transformer_layers, self.latent_seqs)
        
        self.encoder_out_net = ConvBlock(channels, channels, latent_dim)
        
        self.decoder_in_net = ConvBlock(2 * latent_dim, channels, channels)
        
        self.decoder_bottleneck = TransformerBottleneck(channels, num_transformer_layers, self.latent_seqs)
    ...
    def get_x_prop(self, x, prop='s'):
        # Check if the sampled inputs for the given property already exist in x.
        if 'x_p_' + prop + '1' not in x or 'x_p_' + prop + '2' not in x:
            
            # Sample a batch of EEG data where all samples share the same value for the given property.
            # Shape of x1 and x2: (batch_size / (num_categories * 2), 100, 100)
            ...
            # If the loader is in training mode, sample additional batches to fill the total batch size
...
class SplitLatentModel(BaselineModel):
    ...
    def __init__(self, in_channels, channels, latent_dim, num_layers, num_transformer_layers, kernel_size, recon_type=True, content_cosine=False, time_resolution = 256):
        super(SplitLatentModel, self).__init__(in_channels, channels, latent_dim, num_layers, num_transformer_layers, kernel_size, recon_type, content_cosine, time_resolution=time_resolution)
    ...
    def pre_encode(self, x):
        x = self.encoder_in(x) #ConvReLU + ConvBlock
        x = self.encoder(x) 
        x = x.permute(0, 2, 1)
    ...
    def losses(self, x=None, which=None, loader=None):
        # The enabled losses are:
        # 'recon', 
        # 'sub_contra_s', 'task_contra_t', 
        # 'latent_permute_s', 'latent_permute_t', 
        # 'restored_permute_s', 'restored_permute_t', 
        # 'sub_content', 'task_content', 
        # 'sub_cross_s', 'task_cross_t', 
        # 'scramble_permute', 
        # 'conversion_permute', 
        # 'quadruplet_permute_F', 'quadruplet_permute'.
        ...
        loss_dict = {}
        for k in which:
            space = self.determine_space(k) # space can be 's' or 't'
            if space is None:
