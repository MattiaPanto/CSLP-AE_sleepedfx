...
def process_sleepedfx_data(args):
    ...
    for idx, file_pair in enumerate(tqdm(filelist)):
        ...
        # X shape: (n_epochs, n_channels, n_times) (841, 1, 3000)
        X = (epochs.pick(EEG_PICK).get_data() - mu) / std

        Zxx = STFTTransform(fs=SAMPLING_RATE, segment_size=SAMPLING_RATE, step_size=STEP_SIZE, nfft=NFFT)(X.squeeze())[
            :, : SAMPLING_RATE // 2, :
        ]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--nfft', type=int, default=128, help='Number of points for FFT')
