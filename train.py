...
parser.add_argument('--time_resolution', type=int, default=1024) # 200
parser.add_argument('--in_channels', type=int, default=12) #50

parser.add_argument('--channels', type=int, default=256) # 200
parser.add_argument('--num_layers', type=int, default=4) #4
parser.add_argument('--num_transformer_layers', type=int, default=4) #4
parser.add_argument('--latent_dim', type=int, default=64) #64
parser.add_argument('--recon_type', type=str, default='mse')
parser.add_argument('--content_cosine', type=int, default=1)

parser.add_argument('--data_line', type=str, default='simple_data_multi_view') #sleepedfx
...
if __name__ == '__main__':
    ...
    # Load data
    with torch.no_grad():
        data_dict = torch.load(args.data_dir+f"{args.data_line}.pt", weights_only=False, map_location="cuda")
        print("Data loaded")
        
        unique_subjects = data_dict["subjects"].clone().unique().tolist() 
        train_subjects, test_subjects = train_test_split(unique_subjects, test_size=0.3, random_state=SEED, shuffle=True)
        eval_subjects, test_subjects = train_test_split(test_subjects, test_size=0.5, random_state=SEED, shuffle=True)
        
        loader = CustomLoader(data_dict, split_subjects = train_subjects, split = 'train', location = "cpu")
        print(f"Train loader created - {loader.data.nbytes / 1024**2} MB")
        eval_loader = CustomLoader(data_dict, split_subjects = eval_subjects, split = 'eval', location = "cpu")
        print(f"Eval loader created - {eval_loader.data.nbytes / 1024**2 } MB")
        test_loader = CustomLoader(data_dict, split_subjects = test_subjects, split = 'test', location = "cpu")
        print(f"Test loader created - {test_loader.data.nbytes / 1024**2 } MB")
    ...
    model = SplitLatentModel(IN_CHANNELS, args.channels, args.latent_dim, NUM_LAYERS, NUM_TRANSFORMER_LAYERS, KERNEL_SIZE, recon_type=args.recon_type, content_cosine=args.content_cosine, time_resolution=args.time_resolution)
    model.set_losses(
        ...
        for i in pbar:
            ...
            if i % args.eval_every == 0:
                with torch.no_grad():
                    ...
                    total_eval_loss = sum((model.loss_weights[v] * losses[v] for v in list(model.used_losses)))
                    ...
                    eval_results.update({f'eval/total_loss': total_eval_loss})
            ...
            if i % args.save_every == 0 and args.checkpoint and i > 0:
                    checkpoint = {
                        'step': i,
    ...
    plot_until = min([4, IN_CHANNELS])
    for i in range(plot_until):
        for j in range(5):
