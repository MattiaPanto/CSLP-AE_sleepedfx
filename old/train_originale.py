...
from extra.utils_originale import get_results, get_eval_results, get_split_latents, split_do_tsne, plot_latents, CustomLoader, fit_knn_fn, fit_etc_fn
...
parser.add_argument('--in_channels', type=int, default=30) #50
parser.add_argument('--num_layers', type=int, default=4)
parser.add_argument('--latent_dim', type=int, default=64) #40
parser.add_argument('--recon_type', type=str, default='mse')
parser.add_argument('--content_cosine', type=int, default=1)

parser.add_argument('--data_line', type=str, default='simple') #sleepedfx
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        data_dict = torch.load(args.data_dir+f"{args.data_line}_data.pt", weights_only=False)
        
        # NEW
        data_dict["data"].sub_(data_dict["data_mean"]).div_(data_dict["data_std"])
        '''
    ...
    # BATCHES: num of batches
    # EFFECTIVE_BATCH_SIZE: num of samples in each batch
