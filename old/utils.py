...
class SleepCustomLoader():
    
    def __init__(self, data_dict, data_dir, subjects, split='train'):
        self.data_dir = data_dir
        self.split = split

        self.file_paths = np.array([os.path.join(data_dir, data_dict['samples'][i]) for i in range(len(data_dict['samples']))])
        self.size = len(self.file_paths)

        self.subjects = torch.tensor(data_dict["subjects"]).numpy() # List of subject labels
        self.tasks = torch.tensor(data_dict["tasks"]).numpy()      # List of task labels
        self.runs = torch.tensor(data_dict["runs"]).numpy()  #temporary fix for ERN+LRP run labels
        
        #self.data_mean = 0 #data_dict['data_mean'].detach().clone().contiguous().cuda()
        #self.data_std = 0 #data_dict['data_std'].detach().clone().contiguous().cuda()
        
        self.unique_subjects = subjects

        self.cache = OrderedDict() # LRU Cache for already loaded samples
        ...
        self.sample = self.file_paths[sample_indices]

        # Convert to a contiguous array in memory
        self.subjects = np.ascontiguousarray(self.subjects[sample_indices]) 
        self.tasks = np.ascontiguousarray(self.tasks[sample_indices])
        self.runs = np.ascontiguousarray(self.runs[sample_indices])
        self.size = len(self.file_paths)
    ...
    def get_samples(self, samples):
        data = torch.stack([self.load_sample(i) for i in samples])
        return data
    ...
    def sample_by_condition(self, subjects, tasks):
        ...
        return self.get_samples(samples)
    ...
    def sample_by_property(self, property):
        ...
        return samples, self.get_samples(samples), self.subjects[samples], self.tasks[samples], self.runs[samples]
    ...
    def iterator(self):
        for i in range(self.size):
            ...
            yield i, self.load_sample(i), self.subjects[i], self.tasks[i], self.runs[i]
