...
class CustomLoader():
    ...
    def __init__(self, data_dict, split='train'):
        ...
        self.subjects = data_dict["subjects"].numpy() # List of subject labels
        self.tasks = data_dict["tasks"].numpy()       # List of task labels
        ...
        # Assign subjects based on the split
        if split == 'dev':
        ...
        self.task_to_label = data_dict['labels'] #connect task ids to a string
        self.run_labels = ['ERN+LRP', 'MMN', 'N2pc', 'N170', 'N400', 'P3']
        self.unique_tasks = list(self.task_to_label.keys()) # List of 14 items
        self.unique_runs = list(range(len(self.run_labels))) # List of 6 items

        # Filter tasks and runs based on the split
        if split != 'N170':
            # Remove task N170
        ...
        self.subject_indices = {s: [] for s in self.unique_subjects} #A dict with a list for each subject
        self.task_indices = {t: [] for t in self.unique_tasks}       #A dict with a list for each task
        self.run_indices = {r: [] for r in self.unique_runs}         #A dict with a list for each run
        ...
        # For each sample, assign the correct subject, task and run to the dict
        for i, (s, t, r) in enumerate(zip(self.subjects, self.tasks, self.runs)):
