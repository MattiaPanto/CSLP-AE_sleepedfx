...
from utils import get_results, get_eval_results, get_split_latents, split_do_tsne, plot_latents, SleepCustomLoader, fit_knn_fn, fit_etc_fn
...
parser.add_argument('--data_line', type=str, default='sleepedfx') #sleepedfx
...
if __name__ == '__main__':
    ...
    data_dict = torch.load("/root/data/sleepedfx_data.pt", weights_only=False, map_location='cuda')
    unique_subjects = data_dict["subjects"].cpu().unique().tolist()

    train_subjects, test_subjects = train_test_split(unique_subjects, test_size=0.2, random_state=SEED, shuffle=True)
    test_loader = SleepCustomLoader(data_dict, split_subjects = test_subjects, split = 'test')

    KFOLD = 5
    kf = KFold(n_splits=KFOLD, shuffle=True, random_state=SEED)

    for fold, (train_idx, eval_idx) in enumerate(kf.split(train_subjects)):

        print(f"Fold {fold+1}/{KFOLD}", file=sys.stdout, flush=True)
        # Load data
        with torch.no_grad():
            #data_dict["data"].sub_(mean).div_(std)
            train_subjects = np.array(unique_subjects)[train_idx]
            eval_subjects = np.array(unique_subjects)[eval_idx]
            loader = SleepCustomLoader(data_dict, split_subjects = train_subjects, split = 'train')
            eval_loader = SleepCustomLoader(data_dict, split_subjects = eval_subjects, split = 'eval')
            del data_dict
        ...
        with torch.no_grad():
            ...
            eval_results = get_eval_results(subject_latents, task_latents, subjects, tasks, split="eval", kfold_iter=fold)
            eval_results.update({f'eval/{fold}/{v}': losses[v] for v in list(model.used_losses)})
            eval_results.update({f'train/{fold}/{v}': loss_dict[v] for v in list(model.used_losses)})
            eval_results.update({
                f'train/{fold}/total_loss': total_loss,
                f'total_samples': loader.total_samples,
                f'train/{fold}/sched_lr': optimizer.param_groups[0]['lr'],
                f'train/{fold}/s_scale': model.subj_logit_scale.exp().item(),
                f'train/{fold}/t_scale': model.task_logit_scale.exp().item(),
            })
            wandb.log(eval_results)
            if not OLD_SCHED:
                scheduler.step(eval_results[f"XGB/eval/{fold}/task/score"])
